...
let handler = async (m, { conn, args, text, usedPrefix, command, isROwner }) => {
    let _p = usedPrefix
    const linkgc = ''
    const domain = 'https://panel.blackmoonhs.my.id'
    const apikey = 'ptla_WnQEKStbFSzb3ZEDBxg6f5tzYbyDaiSE1kvvhhhgccT8cHOXY4s';
    const c_apikey = 'ptlc_004bn7e6PNHLBwXh2BSg5a263hmQRwgrVBijPkjhvgghxWMDDS'
    switch (command) {
        case 'addusr': {
...
${linkgc}`,
                /*contextInfo: {
                    ...
                    externalAdReply: {
                        title: 'DPANEL HOST',
                        body: `TES`,
                        description: 'DPANEL BOT',
                        mediaType: 1,
                        thumbnail: await (await fetch(await conn.profilePictureUrl('120363042659544903@g.us'))).buffer(),
                        sourceUrl: linkgc,
                        mediaUrl: linkgc,
        ...
        case 'listusr': {
            ...
            await conn.sendMessage(m.chat, {
                text: "Berikut list user *DPANEL HOST*",
...
${usedPrefix + command} name,desc,userId,eggId,locId,memory/disk,cpu`)
            ...
            let f = await fetch(domain + "/api/application/servers", {
                ...
                "body": JSON.stringify({
                    ...
                    "user": usr_id,
                    "egg": parseInt(egg),
                    "docker_image": "ghcr.io/amiruldev20/node:18",
                    "startup": startup_cmd,
                    "environment": {
                        "INST": "npm",
                        "USER_UPLOAD": "0",
...
CREATED AT: ${server.created_at}`)
        ...
        case 'listsrv': {
            ...
            await conn.sendMessage(m.chat, {
                text: "Berikut list server *ZANZ HOST*",
                footer: `Page: ${res.meta.pagination.current_page}/${res.meta.pagination.total_pages}`,
                title: "*ZANZ HOST*",
